...
?>
  ...
  <form action="deal_add_buyer.php" method="post">
    <table border="0">
      <tr>
        <td> Last Name </td>
...
<?php
 ...
 # Process the information from the form displayed
  ...
  try{
   ...
   //insert data
    $db->exec("INSERT INTO buyers (last, first, address, city, state, zipcode,  phone, email) VALUES ('$last', '$first','$address', '$city', '$state', '$zipcode', '$phone', '$email');");
    //get the last id value inserted into the table
    $last_id = $db->lastInsertId();
 //now output the data from the insert to a simple html table...
    print "<h2>Buyer Added</h2>";
    print "<table border=1>";
    print "<tr>";
    print "<td>Last Name</td><td>First Name</td><td>Address</td><td>City</td><td>State</td><td>Zipcode</td><td>Phone</td><td>Email</td>";
    print "</tr>";
    $row = $db->query("SELECT * FROM buyers where id = '$last_id'")->fetch(PDO::FETCH_ASSOC);
    print "<tr>";
    ...
    echo "Errors found in buyer's entry:<br/>";
